...
static inline void _print_synaptic_row(synaptic_row_t synaptic_row) {
	...
	if (synapse_row_plastic_size(synaptic_row) > 0) {
		log_debug("----------------------------------------\n");
		address_t plastic_region_address = synapse_row_plastic_region(
				synaptic_row);
		synapse_dynamics_print_plastic_synapses(plastic_region_address,
				fixed_region_address, ring_buffer_to_input_left_shifts);
	}
	...
	for (uint32_t n = 0; n < n_neurons; n++) {
		for (uint32_t t = 0; t < n_synapse_types; t++) {
			...
			for (uint32_t d = 0; d < (1 << SYNAPSE_DELAY_BITS); d++) {
				empty = empty
						&& (ring_buffers[synapses_get_ring_buffer_index(
...
bool synapses_initialise(address_t synapse_params_address,
	...
	*ring_buffer_to_input_buffer_left_shifts = ring_buffer_to_input_left_shifts;
	...
	uint32_t n_ring_buffer_bits = log_n_neurons + log_n_synapse_types
			+ SYNAPSE_DELAY_BITS;
...
void synapses_do_timestep_update(timer_t time) {
	...
	for (uint32_t neuron_index = 0; neuron_index < n_neurons; neuron_index++) {
		...
		for (uint32_t synapse_type_index = 0;
			...
			uint32_t ring_buffer_index = synapses_get_ring_buffer_index(time,
					synapse_type_index, neuron_index, synapse_type_index_bits,
					synapse_index_bits);

			// Convert ring-buffer entry to input and add on to correct
			// input for this synapse type and neuron
			neuron_add_inputs(synapse_type_index, neuron_index,
